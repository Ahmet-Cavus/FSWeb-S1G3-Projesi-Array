...
function kopyala(dizi) {
  const orjinalKopya = [...dizi];
  return orjinalKopya;
}

const kopyaArr = kopyala(orijinalTatlar);
console.log(kopyaArr);
...
function dizi25Cesitmi(cesitArr) {
  const True = " ";
  const False = "";
  if (cesitArr.length === 25) {
    return true;
  } else {
    return false;
  }
...
function cesitEkle(tatlarArr, yeniTat) {
  tatlarArr.unshift(yeniTat);
  return tatlarArr;
}
...
function sonCesitiKaldir(tatlarArr) {
  tatlarArr.pop();
  return tatlarArr;
}
...
Aşağıdakileri yapmak için aşağıdaki indekstekiCesitiGetir işlevini kullanın:
   ...
   Örneğin: indekstekiCesitiGetir(orijinalTatlar, 2) çalıştırılmasıyla, Kakule'in başarıyla eklendiği 
   varsayarsak sonuç "Ceviz" olucaktır.
*/

function indekstekiCesitiGetir(tatlarr, sayi) {
  return orijinalTatlar[sayi];
}

var tatlarr = [];
console.log(indekstekiCesitiGetir(tatlarr, 2));
...
const newTatlar = ismeGoreCesitCikar(orijinalTatlar, "Tarçın");
console.log(newTatlar);
...
const filtrelenmisTatlar = ismeGoreFiltrele(orijinalTatlar, "Çikolata");
...
function ortalamaKelimeSayisi(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    const kelimeSayisi = array[i].split(" ").length;
    toplamKelimeSayisi += kelimeSayisi;
  }
  const ortalama = toplamKelimeSayisi / array.length;
  return ortalama;
}

const sonuc = ortalamaKelimeSayisi(orijinalTatlar);
console.log("Ortalama kelime sayısı:", sonuc);
...
function rastgeleTatlar(array1, array2, array3, array4) {
  const tumTatlar = [...array1, ...array2, ...array3, ...array4];

  const rastgeleTatlar = [];
  for (let i = 0; i < 25; i++) {
    const rastgeleIndex = Math.floor(Math.random() * tumTatlar.length);
    const rastgeleTat = tumTatlar[rastgeleIndex];
    rastgeleTatlar.push(rastgeleTat);
    tumTatlar.splice(rastgeleIndex, 1);
  }
